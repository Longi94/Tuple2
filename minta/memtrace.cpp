/**
 * \file: memtrace.cpp
 * Memóriakezelési hiba detektor.
 */

/*********************************
Memoriaszivargas-detektor konyvtar
Keszitette: Peregi Tamas
            BME IIT, 2011
            petamas@iit.bme.hu
*********************************/
...
START_NAMESPACE
	...
	static void print_call(const char * msg, call_t call) {
		...
		fprintf(stderr, "%s", pretty[call.f]);
		fprintf(stderr, "%s", call.par_txt ? call.par_txt : "?");
		if (call.f <= 3) fprintf(stderr, ")");
		fprintf(stderr," @ %s:", call.file ? basename(call.file) : "?");
	...
	static void dump_memory(void const *mem, size_t size, size_t can_len) {
		...
		printf("Dump (kanari hossz: %d):\n", can_len);
		size += 2*can_len;
		for (s = 0; s < (size+15)/16; s++) {
			printf("%04x:%c ", s*16, s*16 < can_len || s*16 >= size-can_len ? ' ' : '*');
			for (o = 0; o < 16; o++) {
				if (o == 8) printf(" ");
				if (s*16+o < size)
					printf("%02x ", m[s*16+o]);
			...
			for (o = 0; o < 16; o++) {
				if (s*16+o < size)
					printf("%c", isprint(m[s*16+o]) ? m[s*16+o] : '.');
				else
					printf(" ");
			}
			printf("\n");
	...
	static void die(const char * msg, void * p, size_t size, call_t * a, call_t * d) {
		fprintf(stderr,"%s\n",msg);
		if (p) {
			fprintf(stderr, "\tPointer:\t%p", PU(p));
			if (size) fprintf(stderr," (%u byte)", (unsigned)size);
			fprintf(stderr,"\n");
		}
...
START_NAMESPACE
	...
	static void print_registry_item(registry_item * p) {
		if (p) {
			...
			fprintf(stderr, "\t%p%5u byte ",p->p, (unsigned)p->size);
...
START_NAMESPACE
	...
	static int atexit_class::counter = 0;
#endif
END_NAMESPACE
#endif
